#include "DL_OLED_Font.h"
#include "ti_msp_dl_config.h"

/*引脚配置*/
// #define OLED_W_SCL(x)		DL_GPIO_writePins(GPIOA, DL_GPIO_PIN_29, (x))
// #define OLED_W_SDA(x)		DL_GPIO_writePins(GPIOA, DL_GPIO_PIN_30, (x))

#define OLED_W_SCL(x)   do { if (x) DL_GPIO_setPins(OLED_SCL_PORT, OLED_SCL_PIN_29_PIN); else DL_GPIO_clearPins(OLED_SCL_PORT, OLED_SCL_PIN_29_PIN); } while(0)
#define OLED_W_SDA(x)   do { if (x) DL_GPIO_setPins(OLED_SDA_PORT, OLED_SDA_PIN_30_PIN); else DL_GPIO_clearPins(OLED_SDA_PORT, OLED_SDA_PIN_30_PIN); } while(0)

/* I2C时序延时，50个周期 @32MHz ≈ 1.56µs，对应约320kHz I2C */
#define I2C_DELAY() delay_cycles(50)
...
void OLED_Init(void)
{
	delay_cycles(32000 * 100);	//上电延时 ~100ms @32MHz
